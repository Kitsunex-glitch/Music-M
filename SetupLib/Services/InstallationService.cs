    ...
    public class InstallationService : IInstallationService
    {
        ...
        private async Task InstallApplicationAsync(string packageUrl, PackageType packageType, bool forceInstall, string? PathInstallZIP = null)
        {
            ...
            else // PackageType.ZIP
            {
                ...
                string extractPath = Path.Combine(programFilesPath, "Music M");
        ...
        private string GetZipInstallCommand(string zipPath, string extractPath)
        {
            ...
            sb.AppendLine("# Закрытие процессов Music M перед установкой");
            sb.AppendLine("$processNames = @('Music M', 'MusicM')");
            ...
            sb.AppendLine("# Поиск исполняемого файла Music M.exe в целевой директории");
            sb.AppendLine("$exePath = Get-ChildItem -Path $targetDir -Recurse -Filter \"Music M.exe\" | Select-Object -First 1");
            sb.AppendLine("if (-not $exePath) {");
            sb.AppendLine("    # Если Music M.exe не найден, ищем любое другое .exe");
