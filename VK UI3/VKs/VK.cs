...
namespace VK_UI3.VKs
{
    internal class VK
    {
        ...
        */
        ...
        {
            var (_, hash, _, url, _) = await _authCategory.GetAuthCodeAsync("Music M Player", forceRegenerate);
        ...
        private async Task AuthPasskeyAsync()
        {
            ...
            if (hResult.Succeeded)
            {
                ...
                if (!hResult.Succeeded)
                {
                    login.DialogMessageShow("РќРµ СѓРґР°Р»РѕСЃСЊ РѕС‚РјРµРЅРёС‚СЊ С‚РµРєСѓС‰РёР№ РїСЂРѕС†РµСЃСЃ РІС…РѕРґР°, Music M РЅРµ СЃРјРѕРі РѕС‚РјРµРЅРёС‚СЊ С‚РµРєСѓС‰РёР№ РїСЂРѕС†РµСЃСЃ РІС…РѕРґР°! Р—Р°РєСЂРѕР№С‚Рµ РІСЃРµ РґРёР°Р»РѕРіРё РІС…РѕРґР° СЃ РєР»СЋС‡РµРј Рё РїРѕРІС‚РѕСЂРёС‚Рµ РїРѕРїС‹С‚РєСѓ.");
